...
public class ExampleController {
    ...
    public ResponseEntity<Example> createExample(
            @Parameter(description = "Example data to create") @Valid @RequestBody CreateExampleRequest request) {
    ...
    public ResponseEntity<Example> updateExample(
            ...
            @Parameter(description = "Updated example object") @Valid @RequestBody Example example) {
        return exampleService.findById(id)
                .map(existingExample -> {
                    example.setId(id);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully deleted the example"),
            @ApiResponse(responseCode = "404", description = "Example not found")
    })
    @DeleteMapping("/{id}")
